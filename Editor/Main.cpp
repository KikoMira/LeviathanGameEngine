...
int main(int argcount, char* args[]){
    ...
    Logger mainLog("StandaloneEditor" + std::string("Log.txt"));
    ...
    std::unique_ptr<AppDef> ProgramDefinition(AppDef::GenerateAppdefine(
            "./EngineConf.conf", "./Editor.conf", "./EditorKeys.conf",
            &EditorApplication::CheckGameConfigurationVariables, &EditorApplication::CheckGameKeyConfigVariables));
    ...
    ProgramDefinition->SetMasterServerParameters(MasterServerInformation()).
        ...
        "Leviathan Standalone Editor for version " LEVIATHAN_VERSION_ANSIS, "StandaloneEditor",
        "GAME_VERSIONS");
    
    // Create window last //
    ProgramDefinition->StoreWindowDetails(EditorApplication::GenerateWindowTitle(), true,
#ifdef _WIN32
