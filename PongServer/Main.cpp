...
int main(int argcount, char* args[]){
    ...
    Logger mainLog("PongServer" + std::string("Log.txt"));
    ...
    std::unique_ptr<AppDef> ProgramDefinition(AppDef::GenerateAppdefine(
            "./EngineConf.conf", "./PongServer.conf", "",
            &PongServer::CheckGameConfigurationVariables, &PongServer::CheckGameKeyConfigVariables));
...
#endif
    ProgramDefinition->SetMasterServerParameters(MasterServerInformation("PongServer.txt", "Pong_" GAME_VERSIONS, "http://boostslair.com/", "/Pong/MastersList.txt", "PongCrecentials.txt", false)).
        SetApplicationIdentification(
        "Pong server version " GAME_VERSIONS, "Pong",
        "GAME_VERSIONS");
    
    // Create window last //
    ProgramDefinition->StoreWindowDetails(PongServer::GenerateWindowTitle(), true,
#ifdef _WIN32
